...
export default function AdminDashboard() {
    ...
    const activeStats = statsRaw ? statsRaw[viewMode] : {
        revenue: 0,
        netRevenue: 0,
        orders: 0,
        vat: 0,
    ...
    const statCards = [
        {
            label: viewMode === 'today' ? 'C.A Brut (TTC)' : 'C.A Global (TTC)',
            value: `${(activeStats?.revenue || 0).toFixed(1)}`,
            suffix: 'DT',
            icon: Target,
            trend: 'BRUT',
            color: 'text-white',
        ...
        {
            label: viewMode === 'today' ? 'Revenu Net (HT)' : 'Net Global (HT)',
            value: `${(activeStats?.netRevenue || 0).toFixed(1)}`,
            suffix: 'DT',
            icon: DollarSign,
            trend: 'RÉEL',
        ...
        {
            label: viewMode === 'today' ? 'Fiscalité (Interne)' : 'Fiscalité Totale',
            value: `${((activeStats?.vat || 0) + (activeStats?.stampDuty || 0)).toFixed(1)}`,
            suffix: 'TAX',
            icon: ShieldAlert,
            trend: 'TVA + TIMBRE',
        ...
        {
            label: viewMode === 'today' ? 'Unités 24H' : 'Total Unités',
            value: (activeStats?.orders || 0) + (activeStats?.reservations || 0),
            suffix: 'OPS',
            icon: Zap,
            trend: `${activeStats?.orders || 0} CMD / ${activeStats?.reservations || 0} RES`,
    ...
    return (
        <div className="w-full space-y-12 animate-in fade-in duration-700 pb-20">
            ...
            <div className="flex flex-col xl:flex-row justify-between items-start xl:items-end gap-10">
                ...
                <div className="flex items-center gap-6 self-end xl:self-auto w-full xl:w-auto">
                    {/* Perspective Toggle */}
                    <div className="bg-white/[0.02] border border-white/5 p-1.5 rounded-[1.5rem] flex items-center gap-1 shadow-2xl">
                        {(['today', 'global'] as const).map((mode) => (
                            <button
                                key={mode}
                                onClick={() => setViewMode(mode)}
                                className={`px-8 py-3.5 rounded-2xl text-[10px] font-[1000] uppercase tracking-[0.2em] transition-all duration-500 italic ${viewMode === mode
                                    ? 'bg-yellow-400 text-black shadow-[0_0_30px_rgba(250,204,21,0.3)]'
                                    : 'text-gray-600 hover:text-white hover:bg-white/5'
                                    }`}
                            >
                                {mode === 'today' ? 'Aujourd\'hui' : 'Global'}
                            </button>
                        ))}
                    </div>
                        ...
                        <div className="relative z-10 space-y-8">
                            ...
                            <div>
                                ...
                                <div className="flex items-baseline gap-3">
                                    <span className="text-5xl font-[1000] italic tracking-tighter text-white leading-none">{stat.value}</span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                {[
                    { label: 'TOTAL TRANSMISSIONS', value: statsRaw?.global?.orders || 0, icon: ShoppingBag, link: '/dashboard/orders' },
                    { label: 'RÉSERVATIONS JOUR', value: statsRaw?.today?.reservations || 0, icon: Users, link: '/dashboard/reservations' },
                                    ...
                                    recentOrders.map((order) => (
                                        ...
                                        >
                                            <td className="px-12 py-10">
                                                <span className="font-[1000] text-yellow-400 italic text-3xl tracking-tighter leading-none group-hover:scale-110 transition-transform origin-left block">#{order.order_number}</span>
                                            </td>
                                            <td className="px-6 py-10">
                                                <div className="space-y-1">
                                                    <p className="text-white font-[1000] text-sm uppercase tracking-widest group-hover:text-yellow-400 transition-colors leading-none">{order.delivery_info?.full_name || 'Client Anonyme'}</p>
                                                    <p className="text-[10px] text-gray-700 font-bold tracking-[0.2em] uppercase">{order.delivery_info?.phone || 'Pas de numéro'}</p>
                                                </div>
                                            </td>
                                            <td className="px-6 py-10">
                                                <div className="flex items-baseline gap-2">
                                                    <span className="text-white font-[1000] text-2xl italic tracking-tighter leading-none">{(order.total_amount || 0).toFixed(1)}</span>
                                            ...
                                            <td className="px-12 py-10 text-right">
                                                <div className="space-y-1">
                                                    <p className="text-[10px] text-white font-[1000] uppercase tracking-[0.3em] italic leading-none">{new Date(order.created_at || order.createdAt).toLocaleDateString('fr-FR', { day: '2-digit', month: 'short' })}</p>
                                                    <p className="text-[10px] text-gray-700 font-bold uppercase tracking-[0.4em] italic leading-none">{new Date(order.created_at || order.createdAt).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' })}</p>
                                                </div>
                                            </td>
                                        </tr>
                                    ))
