...
import { SupportModalProvider } from '@/hooks/useSupportModal';
                    ...
                    <main ref={mainRef} className="flex-1 overflow-y-auto custom-scrollbar relative">
                        <header className={`sticky top-0 z-40 transition-all duration-700 flex items-center justify-between px-6 md:px-12 ${scrolled
                            ...
                            <div className="flex items-center gap-4">
                                <Link
                                    href="/account/cart"
                                    className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all relative border ${pathname === '/account/cart' ? 'bg-yellow-400 text-black border-transparent' : 'bg-white/5 text-gray-400 hover:text-white hover:bg-white/10 border-white/5'}`}
                                >
                                    <ShoppingBag size={18} />
                                    {totalItems > 0 && (
                                        <motion.span
                                            initial={{ scale: 0 }}
                                            animate={{ scale: 1 }}
                                            className="absolute -top-1.5 -right-1.5 w-4.5 h-4.5 bg-yellow-400 text-black text-[9px] font-black rounded-full flex items-center justify-center border-2 border-[#050505] shadow-lg"
                                        >
                                            {totalItems}
                                        </motion.span>
                                    )}
